...
function printExcel() {
    let tableData = document.getElementById("tableActualAnpPerBulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Actual Polis dan ANP Per Bulan' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceActualPolisANPBulan/` + guid;
}
 ...
 * @returns LIST
 */
async function getActualAnpPerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-actual-anp-per-bulan?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListActualAnpPerBulan() {
    const req = await getActualAnpPerBulan();
    const result = req.ActualPolisAnpPerBulanResult;
    ...
    const tbody = document.querySelector('#tableActualAnpPerBulan tbody');
