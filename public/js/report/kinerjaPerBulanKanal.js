...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListKinerjaPerbulanKanal();
...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaPerbulanKanal").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Perbulan Kanals' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaPerBulanKanal/`+guid;
}
...
/**
 * GET DATA KINERJA PERBULAN KANAL
 * @returns LIST
 */
async function getKinerjaPerbulanKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-perbulan-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListKinerjaPerbulanKanal() {
    const req = await getKinerjaPerbulanKanal();
    const result = req.KinerjaPerbulanKanalResult;
    ...
    const tbody = document.querySelector('#tableKinerjaPerbulanKanal tbody');
