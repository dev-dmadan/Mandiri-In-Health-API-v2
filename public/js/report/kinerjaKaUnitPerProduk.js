...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListKinerjaProdukPerKaUnit();
    renderFilterTarget();
    renderFilterKanit();
    // renderFilterAgen();
    renderFilterPeriode();
...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaKaUnitPerProduk").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Kepala Unit Per Produk' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaKaUnitPerProduk/` + guid;
}
 ...
 * @returns LIST
 */
async function getKinerjaProdukPerKaUnit() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-peroduk-per-kanit?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListKinerjaProdukPerKaUnit() {
    const req = await getKinerjaProdukPerKaUnit();
    const result = req.KinerjaProdukPerKanitResult;
    ...
    const tbody = document.querySelector('#tableKinerjaKaUnitPerProduk tbody');
    
    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<th style="border: 1px solid; text-align: center;  background:#EEEEEE; font-weight: normal;">${seqNo}</th>` +
            `<th style="border: 1px solid; text-align: left;  background:#EEEEEE; font-weight: normal;">${item.Name.toUpperCase()}</th>` +
            `<th style="border: 1px solid; text-align: left;  background:#EEEEEE; font-weight: normal;">${item.Item1}</th>` +
            ...
            `<td style="border: 1px solid; text-align: center;  background:#EEEEEE;">${item.Item4}%</td>` +
            `<td  style="border: 1px solid; text-align: center;  background:#EEEEEE;"><div style="background-color: ${progressBarColor(item.Item4, item.Item3)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center;  background:#EEEEEE;">${item.Item7}%</td>` +
            `<td style="border: 1px solid; text-align: center;  background:#EEEEEE;"><div style="background-color: ${progressBarColor(item.Item7, item.Item6)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center;  background:#EEEEEE;">${item.Item10}%</td>` +
            `<td style="border: 1px solid; text-align: center;  background:#EEEEEE;"><div style="background-color: ${progressBarColor(item.Item10, item.Item9)};border-radius:50%;width:15px;height:15px;"></div></td>`
        let tr = document.createElement('tr');
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tableKinerjaKaUnitPerProduk tfoot');
    let rowsTotal =
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" colspan="3">TOTAL</th>` +
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: right; color:white; font-weight:bold">${result.Total.Item1}</td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item3, result.Total.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: right; color:white; font-weight:bold">${result.Total.Item5}</td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item6, result.Total.Item5)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item9, result.Total.Item8)};border-radius:50%;width:15px;height:15px;"></div></td>`
