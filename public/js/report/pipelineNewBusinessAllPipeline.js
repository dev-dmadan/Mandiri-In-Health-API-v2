...
document.addEventListener("DOMContentLoaded", function () {
    ...
    renderListPipelineNewBusinessAllPipeline();
...
function printExcel() {
    let tableData = document.getElementById( "tablePipelinenNewBusinessAllPipeline").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement("a");
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`;
    a.download = "Daftar Pipeline New Business-All Pipeline" + ".xls";
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBDaftarAllPipeline/` + guid;
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessAllPipeline() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(
            `${APP_URL}/dashboard/api/report-get-pipeline-newbusiness-all-pipeline?SecretKey=${SECRET_KEY}`,
            {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getPipelineNewBusinessAllPipeline();
        const result = req.DaftarPipelineNewBusinessAllPipelineResult;
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessAllPipeline() {
    const req = await getPipelineNewBusinessAllPipeline();
    const result = req.DaftarPipelineNewBusinessAllPipelineResult;
    ...
    const tbody = document.querySelector("#tablePipelinenNewBusinessAllPipeline tbody");

    let seqNo = 1;
    result.ListItem.forEach((item) => {
        let rows =
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-center freeze-table">${seqNo}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-left">${item.Name.toUpperCase()}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-center">${item.Item1}</th>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item5}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-right">${item.Item9}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-right">${item.Item10}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-right">${item.Item11}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item12}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item13 == "False" ? "Tidak" : "Ya"}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item14}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item15}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item16}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item17 == "01-01-0001" ? "-" : item.Item17}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item20}</td>`;
        let tr = document.createElement("tr");
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tablePipelinenNewBusinessAllPipeline tfoot');
    let rowsTotal =
