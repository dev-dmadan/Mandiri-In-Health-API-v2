...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaBisnis").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Bisnis' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaBisnis/`+ guid;
}
 ...
 * @returns LIST
 */
async function getLaporanKinerjaBisnis() {
    try {
       
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-bisnis?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListKinerjaBisnis() {
    const req = await getLaporanKinerjaBisnis();
    const result = req.KinerjaBisnisResult;

    if (!result.Status.Success) {
        throw result.Status.Message;
    }

    const tbody = document.querySelector('#tableKinerjaBisnis tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td  width="1%" style="border: 1px solid; width:1%" class="text-center"><div style="background-color: ${progressBarColor(item.Item3, item.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>` +
    ...
    let rowsTotal =
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold">TOTAL</td>` +
        ...
        `<td  width="1%" style="border: 1px solid; background-color: #4472C4;" class="text-center"><div style="align: center; background-color: ${progressBarColor(result.Total.Item3, result.Total.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>` +
    ...
    // trTotal.style.backgroundColor = "#1e88d3";
