...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListActualProposalPerBulan();
...
function printExcel() {
    let tableData = document.getElementById("tableActualProposalPerBulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Actual Proposal Per Bulan' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceActualProposalPerBulan/` + guid;
}
 ...
 * @returns LIST
 */
async function getActualProposalPerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-actual-proposal-per-bulan?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListActualProposalPerBulan() {
    const req = await getActualProposalPerBulan();
    const result = req.ActualProposalPerBulanResult;
    ...
    const tbody = document.querySelector('#tableActualProposalPerBulan tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            // `<td style="border: 1px solid;" class="text-right">${item.Item3}</td>` 
