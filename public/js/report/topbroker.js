...
function printExcel() {
    let tableData = document.getElementById("tableTopBroker").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'TOP 10 BROKER' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportTopTenGWPBroker/` + guid;
}
...
async function getTopBroker() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-top-broker?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderFilterProduct() {
    try {
        ...
        dataProduk = dataProduk.filter(item => item.IsActive == true);
 ...
 * @returns VIEW
 */
async function renderListTopBroker() {
    const req = await getTopBroker();
    const result = req.ReportBrokerResult;
    ...
    const tbody = document.querySelector('#tableTopBroker tbody');
