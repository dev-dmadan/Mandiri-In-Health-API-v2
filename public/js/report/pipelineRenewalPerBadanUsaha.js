...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListRekapPipelinePerBadanUsaha();
...
function printExcel() {
    let tableData = document.getElementById("tablePipelineRenewalPerBadanUsaha").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Daftar Pipeline Renewal Per Badan Usaha' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineRNDaftarPerBU/` + guid;
}
 ...
 * @returns LIST
 */
async function getRekapPipelinePerBadanUsaha() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-pipeline-renewal-per-badan-usaha?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getRekapPipelinePerBadanUsaha();
        const result = req.PipelineRenewalPerBadanUsahaResult;
 ...
 * @returns VIEW
 */
async function renderListRekapPipelinePerBadanUsaha() {
    const req = await getRekapPipelinePerBadanUsaha();
    const result = req.PipelineRenewalPerBadanUsahaResult;
    ...
    const tbody = document.querySelector('#tablePipelineRenewalPerBadanUsaha tbody');

    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<td style="border: 1px solid;" class="text-center">${item.Item14}</td>` +
            `<td style="border: 1px solid;" class="text-left">${item.Name.toUpperCase()}</td>` +
            `<td style="border: 1px solid;" class="text-center">${item.Item1}</td>` +
            `<td style="border: 1px solid;" class="text-left">${item.Item2}</td>` +
            ...
            `<td style="border: 1px solid;" class="text-left">${item.Item12}</td>` +
            `<td style="border: 1px solid;" class="text-center">${item.Item13}</td>` 
    ...
    let rowsTotal =
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" colspan="6">TOTAL</td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold" colspan="6"></td>`;
