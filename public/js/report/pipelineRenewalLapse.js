...
document.getElementById ("progress").addEventListener ("change", onChangeProgress, false);
// document.getElementById("printButtonPdf").addEventListener("click", createPDF, false);
...
var PROGRESS = localStorage['Progress_Report'];
...
function printExcel() {
    let tableData = document.getElementById("tablePipelineRenewalLapse").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'DAFTAR BADAN USAHA RENEWAL LAPSE' + '.xls'
    a.click();
}

function createPDF() {
    
    var sTable = document.getElementById('tablePipelineRenewalLapse').outerHTML;
     // add all the css and styling for the table 
    var style = "<style>";
    style = style + "table {width: 100%;font: 12px Calibri;}";
    // style = style + "table, td {border: solid 1px #000; border-collapse: collapse;";
    style = style + "table, th {border: solid 1px #000; border-collapse: collapse; background-color:#4472c4";
    style = style + "padding: 5px 10px;text-align: center;}";
    style = style + "</style>";
    ...
    win.document.write('<title>DAFTAR BADAN USAHA RENEWAL LAPSE</title>');   // title or name for pdf
...
function onChangeProgress() {
    ...
    localStorage['Progress_Report'] = progress;
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineRNDaftarLapse/` + guid;
}
 ...
 * @returns LIST
 */
async function getRekapPipelineLapse() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-pipeline-renewal-lapse?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
...
/**
 * RENDER FILTER PROGRESS
 * @returns VIEW
 */
async function renderFilterProgress() {
    try {
        const data = [
            "Follow up Renewal",
            "Follow up Penawaran",
        ...
        const selectOption = document.querySelector('#progress');
        var opt = document.createElement('option');
        opt.value = "";
        opt.innerHTML = "ALL";
        opt.selected = true;
...
async function renderPageNumber() {
    try {
        const req = await getRekapPipelineLapse();
        const result = req.PipelineRenewalLapseResult;
...
/**
 * RENDER LIST PIPELINE LAPSE
 * @returns VIEW
 */
async function renderListPipelineLapse() {
    const req = await getRekapPipelineLapse();
    const result = req.PipelineRenewalLapseResult;
    ...
    const tbody = document.querySelector('#tablePipelineRenewalLapse tbody');

    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-center">${seqNo}</th>` +
        //  `<td style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-left">${item.Name.toUpperCase()}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-left">${item.Item1}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-wrap text-left">${item.Item2}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-right">${item.Item5}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-right">${item.Item6}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item12}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item13}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item14}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-left">${item.Item15}</td>` +
    ...
    const tfoot = document.querySelector('#tablePipelineRenewalLapse tfoot');
    let rowsTotal =
