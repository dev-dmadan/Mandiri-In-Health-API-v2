...
function reload() {
    window.location = `${APP_URL}/report/view/performanceTargetKanalImpor/` + guid;
}
...
async function getTargetPerKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-target-per-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderListTargetPerKanal() {
    ...
    const result = req.TargetKanalResult;
