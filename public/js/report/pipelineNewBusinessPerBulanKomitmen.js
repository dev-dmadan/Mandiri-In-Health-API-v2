...
function printExcel() {
    let tableData = document.getElementById("tablePipelineNewBusinessPerBulanKomitemen").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Rekap Progress Pipeline New Business Per Bulan (Komitmen)' + '.xls'
    a.click();
}
...
async function getProgressPipelineKomitmenPerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-progress-pipeline-newbusiness-komitemen?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderListProgressPipelineKomitmenPerBulan() {
    ...
    const result = req.ProgressPipelineNewBusinessKomitenPerBulanResult;
    ...
    const tbody = document.querySelector('#tablePipelineNewBusinessPerBulanKomitemen tbody');
    ...
    const tfoot = document.querySelector('#tablePipelineNewBusinessPerBulanKomitemen tfoot');
    let rowsTotal =
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"></th>` +
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold">TOTAL</th>` +
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold">${result.Total.Item19}</th>` + 
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold">${result.Total.Item20}</th>` + 
