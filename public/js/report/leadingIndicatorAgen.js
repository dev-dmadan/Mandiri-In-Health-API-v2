...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListLeadingIndicatorAgen();
...
function printExcel() {
    let tableData = document.getElementById("tableLeadingIndicatorAgen").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Leading Indicator Agen' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportLeadingIndicatorAgen/` + guid;
}
...
/**
 * GET DATA LEADING INDICATOR AGEN 
 * @returns LIST
 */
async function getLeadingIndicatorAgen() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-leading-indicator-agen?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListLeadingIndicatorAgen() {
    const req = await getLeadingIndicatorAgen();
    const result = req.LeadingIndicatorNewBusinessAgenResult;

    if (!result.Status.Success) {
        throw result.Status.Message;
    }

    const tbody = document.querySelector('#tableLeadingIndicatorAgen tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td width="1%" style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item3)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td width="1%" style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item6)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td width="1%" style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item9)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td width="1%" style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item12)};border-radius:50%;width:15px;height:15px;"></div></td>`
        let tr = document.createElement('tr');
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tableLeadingIndicatorAgen tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold">${result.Total.Item2}</td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item3)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold">${result.Total.Item4}</td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold">${result.Total.Item6}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item6)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: right; color:white; font-weight:bold">${result.Total.Item7}</td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item9)};border-radius:50%;width:15px;height:15px;"></div></td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: right; color:white; font-weight:bold">${result.Total.Item11}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold">${result.Total.Item12}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; z-index:0; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item12)};border-radius:50%;width:15px;height:15px;"></div></td>`
