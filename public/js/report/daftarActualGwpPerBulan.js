...
function printExcel() {
    let tableData = document.getElementById("tableDaftarActualGwpPerBulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement("a");
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`;
    a.download = "Daftar Actual GWP Per Bulan" + ".xls";
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceDaftarActualGWPPerBulan/` + guid;
}
 ...
 * @returns LIST
 */
async function getDaftarActualGwp() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(
            `${APP_URL}/dashboard/api/report-get-daftar-actual-gwp?SecretKey=${SECRET_KEY}`,
            {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getDaftarActualGwp();
        const result = req.DaftarActualGwpResult;
        let dataSeries = [];
 ...
 * @returns VIEW
 */
async function renderDaftarActualGwp() {
    const req = await getDaftarActualGwp();
    const result = req.DaftarActualGwpResult;
    ...
    const tbody = document.querySelector("#tableDaftarActualGwpPerBulan tbody");
    ...
    result.ListItem.forEach((item) => {
        let rows =
            ...
            `<th class="text-wrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: left">${item.Item3}</th>` +
            `<th class="text-wrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: center">${item.Item4}</th>` +
            ...
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item7}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item8}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item9}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item10}</td>` +
    ...
    const tfoot = document.querySelector('#tableDaftarActualGwpPerBulan tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0" colspan="8"></td>`
