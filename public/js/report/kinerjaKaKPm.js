...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaKAKPM").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Kepala Kanal' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaKaKPm`;
}
 ...
 * @returns LIST
 */
async function getKinerjaKAKPM() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-ka-kpm?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
 ...
 * @returns VIEW
 */
async function renderListKinerjaKAKPM() {
    const req = await getKinerjaKAKPM();
    const result = req.KinerjaKaKPMResult;
    console.log(result);
    ...
    const tbody = document.querySelector('#tableKinerjaKAKPM tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td style="border: 1px solid; text-align: left">${item.Item1 != null ? item.Item1 : "-"}</td>` +
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item4)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item7)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item10)};border-radius:50%;width:15px;height:15px;"></div></td>`
