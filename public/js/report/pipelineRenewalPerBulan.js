...
function printExcel() {
    let tableData = document.getElementById("tablePipelineRenewalPerBulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'REKAP PROGRESS PIPELINE RENEWAL PER BULAN' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineRNRekapPerBulan/` + guid;
}
...
async function getRekapPipelinePerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-rekap-progress-pepeline-renewal-perbulan?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderListRekapPipelinePerBulan() {
    ...
    const result = req.ProgressPipelineRenewalPerBulanResult;
    ...
    const tbody = document.querySelector('#tablePipelineRenewalPerBulan tbody');
