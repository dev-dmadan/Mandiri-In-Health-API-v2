...
var BULANINT = localStorage["BulanInt_Report"];
...
function isFilterKanalVisible(visible = true) {
    if (!visible) {
        ...
        const tipefilter = document.querySelector('#tipefilter');
        tipefilter.remove();
...
function printExcel() {
    let tableData = document.getElementById("tableBuInforce").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement("a");
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`;
    a.download = "Daftar Bu Inforce" + ".xls";
    a.click();
}
...
function onChangeBulan() {
    ...
    localStorage["BulanInt_Report"] = bulanId;
    console.log(bulanId);
    reload();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportDaftarBuInforce/` + guid;
}
...
async function getKanalDistribusi() {
    try {
        ...
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-filter-kanal-bu-inforce?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
 ...
 * @returns LIST
 */
async function getBuInforce() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(
            `${APP_URL}/dashboard/api/report-get-bu-inforce?SecretKey=${SECRET_KEY}`,
            {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
async function renderFilterMonth() {
    try {
        ...
        selectOption.value = BULANINT;
        console.log(BULANINT);
    } catch (error) {
        throw error;
    }
}
...
async function renderFilterTipeKinerja() {
    try {
        ...
        result.ListTipeKinerja.forEach((item) => {
            var opt = document.createElement("option");
            opt.value = item.Id;
            opt.innerHTML = item.Name;
            selectOption.appendChild(opt);
...
async function renderPageNumber() {
    try {
        const req = await getBuInforce();
        const result = req.GetReportBuInforceResult;
        let dataSeries = [];
 ...
 * @returns VIEW
 */
async function renderBuInforce() {
    const req = await getBuInforce();
    const result = req.GetReportBuInforceResult;
    ...
    const tbody = document.querySelector("#tableBuInforce tbody");

    let seqNo = 1;
    result.ListItem.forEach((item) => {
        let rows =
            `<th class="text-nowrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: center">${seqNo}</th>` +
            `<th class="text-nowrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: left">${item.Item1}</th>` +
            `<th class="text-nowrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: center">${item.Item2}</th>` +
            `<th class="text-nowrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: center">${item.Item3}</th>` +
            `<th class="text-wrap" style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: left">${item.Item4}</th>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item5}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item6}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item7}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item8}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item9}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item10}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item11}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item12}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item13}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item14}</td>` +
            `<td class="text-wrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item15}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item16}</td>` +
            `<td class="text-nowrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item17}</td>` +
            `<td class="text-wrap" style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item18}</td>` 
        let tr = document.createElement("tr");
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tableBuInforce tfoot');
    let rowsTotal =
        `<th style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" colspan="5">TOTAL</th>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item3}</td>`+
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0" colspan="7"></td>`
