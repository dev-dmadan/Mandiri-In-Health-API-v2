...
function printExcel() {
    let tableData = document.getElementById("tableDaftarPolisAnp").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Daftar Polis dan ANP' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceDaftarPolisANP/` + guid;
}
 ...
 * @returns LIST
 */
async function getDaftarPolisAnp() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-daftar-polis-anp?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getDaftarPolisAnp();
        const result = req.DaftarPolisAnpResult;
        let dataSeries = [];
 ...
 * @returns VIEW
 */
async function renderDaftarPolisAnp() {
    const req = await getDaftarPolisAnp();
    const result = req.DaftarPolisAnpResult;
    ...
    const tbody = document.querySelector('#tableDaftarPolisAnp tbody');
    ...
    const tfoot = document.querySelector('#tableDaftarPolisAnp tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0" colspan="3"></td>`
