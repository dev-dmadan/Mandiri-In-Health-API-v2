...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListPipelineNewBusinessLoss();
...
function printExcel() {
    let tableData = document.getElementById("tabelPipelineNewBusinessLoss").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Daftar Pipeline New Business Loss' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBDaftarLoss/` + guid;
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessLoss() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-pipeline-newbusiness-loss?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderPageNumber() {
    try {
        const req = await getPipelineNewBusinessLoss();
        const result = req.PipelineNewBusinessLossResult;
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessLoss() {
    const req = await getPipelineNewBusinessLoss();
    const result = req.PipelineNewBusinessLossResult;
    ...
    const tbody = document.querySelector('#tabelPipelineNewBusinessLoss tbody');
    ...
    const tfoot = document.querySelector('#tabelPipelineNewBusinessLoss tfoot');
    let rowsTotal =
