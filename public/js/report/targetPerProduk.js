...
function reload() {
    window.location = `${APP_URL}/report/view/performanceTargetProdukImpor/` + guid;
}
 ...
 * @returns LIST
 */
async function getTargetPerProduk() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-target-per-produk?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListTargetPerProduk() {
    const req = await getTargetPerProduk();
    const result = req.TargetProdukResult;
    ...
    const tbody = document.querySelector('#tableTargetProduk tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-right">${item.Item13}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-right">${item.Item17}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-right">${item.Item21}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-right">${item.Item25}</td>` +
    ...
    const tfoot = document.querySelector('#tableTargetProduk tfoot');
    let rowsTotal =
