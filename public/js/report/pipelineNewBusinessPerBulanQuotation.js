...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListProgressPipelineKomitmenPerBulan();
...
function printExcel() {
    let tableData = document.getElementById("tablePipelineNewBusinessPerBulanQuotation").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Rekap Progress Pipeline New Business Per Bulan (Quotation)' + '.xls'
    a.click();
}

function createPDF() {
    
    var sTable = document.getElementById('tablePipelineNewBusinessPerBulanQuotation').outerHTML;
    ...
    style = style + "table, th { color:black; border: solid 1px #black; border-collapse: collapse; background-color:#4472c4";
    ...
    win.document.write('<title>REKAP PROGRESS PIPELINE QUOTATIO PER BULAN</title>');   // title or name for pdf
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBRekapProgressPipelineQuotation/` + guid;
}
 ...
 * @returns LIST
 */
async function getProgressPipelineKomitmenPerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-progress-pipeline-newbusiness-quotation?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListProgressPipelineKomitmenPerBulan() {
    const req = await getProgressPipelineKomitmenPerBulan();
    const result = req.ProgressPipelineNewBusinessQuotationPerBulanResult;
    ...
    const tbody = document.querySelector('#tablePipelineNewBusinessPerBulanQuotation tbody');
