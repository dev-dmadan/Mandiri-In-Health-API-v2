...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaPerbulanAgen").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Perbulan Agen' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaPerBulanAgen/` + guid;
}
...
/**
 * GET DATA KINERJA PERBULAN AGEN
 * @returns LIST
 */
async function getKinerjaPerbulanAgen() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-perbulan-agen?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
/**
 * RENDER LIST DETAIL KINERJA PERBULAN AGEN
 * @returns VIEW
 */
async function renderListKinerjaPerbulanAgen() {
    const req = await getKinerjaPerbulanAgen();
    const result = req.KinerjaPerbulanAgenResult;
    ...
    const tbody = document.querySelector('#tableKinerjaPerbulanAgen tbody');
    
    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<td style="border: 1px solid; text-align: center">${seqNo}</td>` +
            `<td style="border: 1px solid; text-align: left" class="">${item.Name}</td>` +
            `<td style="border: 1px solid; text-align: right">${item.Item1}</td>` +
            ...
            `<td  width="1%" style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item9, item.Item8)};border-radius:50%;width:15px;height:15px;"></div></td>`
    ...
    let rowsTotal =
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" colspan="2" class="text-center">TOTAL</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item1}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item2}</td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item4}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item5}</td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item7}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold" class="text-right">${result.Total.Item8}</td>`+ 
