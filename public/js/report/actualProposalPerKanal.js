...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListActualProposalPerKanal();
...
function printExcel() {
    let tableData = document.getElementById("tableActualProposalPerKanal").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Actual Proposal Per Kanal' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceActualProposalPerKanal/` + guid;
}
 ...
 * @returns LIST
 */
async function getActualProposalPerKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-actual-proposal-per-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListActualProposalPerKanal() {
    const req = await getActualProposalPerKanal();
    const result = req.ActualProposalPerKanalResult;
    ...
    const tbody = document.querySelector('#tableActualProposalPerKanal tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center">${item.Item4}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center">${item.Item8}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center">${item.Item12}</td>` +
    ...
    const tfoot = document.querySelector('#tableActualProposalPerKanal tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item1}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item2}</td>` +
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item5}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item6}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item7}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item8}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item9}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item10}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item11}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item12}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item13}</td>` 
