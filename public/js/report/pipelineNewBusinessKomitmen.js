...
document.addEventListener("DOMContentLoaded", function () {
    ...
    renderListPipelineNewBusinessKomitmen();
...
function printExcel() {
    let tableData = document.getElementById("tabelPipelineNewBusinessKomitmen").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement("a");
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`;
    a.download = "Daftar Pipeline New Business Komitmen" + ".xls";
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBDaftarPipelineKomitment/` +guid;
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessKomitmen() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(
            `${APP_URL}/dashboard/api/report-get-pipeline-newbusiness-komitmen?SecretKey=${SECRET_KEY}`,
            {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
async function renderFilterProduct() {
    try {
        ...
        const dataProduk = result.ListProduct.filter(
...
async function renderFilterKanit() {
    try {
        ...
        if (KANALDISTRIBUSIID == "00000000-0000-0000-0000-000000000000") {
            ...
            dataKanit = dataKanit.filter(
                (item) => item.KanalSekarangId == KANALDISTRIBUSIID
            );
        }
...
async function renderFilterAgen() {
    try {
        ...
        if (KANALDISTRIBUSIID == "00000000-0000-0000-0000-000000000000") {
            ...
            dataAgen = dataAgen.filter(
                (item) => item.KanalDistribusiId == KANALDISTRIBUSIID
            );
        }
...
async function renderPageNumber() {
    try {
        const req = await getPipelineNewBusinessKomitmen();
        const result = req.DaftarPipelineNewBusinessKomitmenResult;
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessKomitmen() {
    const req = await getPipelineNewBusinessKomitmen();
    const result = req.DaftarPipelineNewBusinessKomitmenResult;
    ...
    const tbody = document.querySelector(
        "#tabelPipelineNewBusinessKomitmen tbody"
    );
    ...
    result.ListItem.forEach((item) => {
        let rows =
            ...
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-left">${item.Name.toUpperCase()}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-center">${item.Item1}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-left">${item.Item2}</th>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item3}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item4}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item5}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item6}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item7}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item8}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-right">${item.Item9}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item12}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item13 == "False" ? "Tidak" : "Ya"}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item14}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item15}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-left text-nowrap">${item.Item16}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-center text-nowrap">${item.Item17}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-left text-nowrap">${item.Item18}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-left text-nowrap">${item.Item19}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-left text-nowrap">${item.Item20}</td>`;
        let tr = document.createElement("tr");
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tabelPipelineNewBusinessKomitmen tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold; z-index:0">${result.Total.Item4}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0"></td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0"></td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0"></td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0"></td>` +
