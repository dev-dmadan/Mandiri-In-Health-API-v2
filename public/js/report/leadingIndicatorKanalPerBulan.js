...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListLeadingIndicatorKanalPerbulan();
...
function printExcel() {
    let tableData = document.getElementById("tableLeadingIndicatorKanalPerbulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Leading Indicator Kanal Per Bulan' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportLeadingIndicatorKanalPerBulan/`+ guid;
}
...
/**
 * GET DATA LEADING INDICATOR KANAL PER BULAN
 * @returns LIST
 */
async function getLeadingIndicatorKanalPerbulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-leading-indicator-kanal-perbulan?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListLeadingIndicatorKanalPerbulan() {
    const req = await getLeadingIndicatorKanalPerbulan();
    const result = req.LeadingIndicatorNewBusinessKanalPerbulanResult;

    if (!result.Status.Success) {
        throw result.Status.Message;
    }

    const tbody = document.querySelector('#tableLeadingIndicatorKanalPerbulan tbody');
