...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListPipelineNewBusinessQuotation();
...
function printExcel() {
    let tableData = document.getElementById("tabelPipelineNewBusinessQuotation").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Daftar Pipeline New Business Quotation' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBDaftarPipelineQuotation/` + guid;
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessQuotation() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-pipeline-newbusiness-quotation?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getPipelineNewBusinessQuotation();
        const result = req.DaftarPipelineNewBusinessQuotationResult;
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessQuotation() {
    const req = await getPipelineNewBusinessQuotation();
    const result = req.DaftarPipelineNewBusinessQuotationResult;
    ...
    const tbody = document.querySelector('#tabelPipelineNewBusinessQuotation tbody');

    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal;" class="text-nowrap text-center">${item.Item21}</th>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-centre">${item.Item8}</td>` +
            ...
            `<td style="border: 1px solid; background:#EEEEEE;" class="text-nowrap text-center">${item.Item17}</td>` +
    ...
    const tfoot = document.querySelector('#tabelPipelineNewBusinessQuotation tfoot');
    let rowsTotal =
