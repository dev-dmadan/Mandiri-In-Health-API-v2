...
function printExcel() {
    let tableData = document.getElementById("tableRekapGwpBadanUsaha").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement("a");
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`;
    a.download = "Rekap Gwp Bandan Usaha" + ".xls";
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/performanceRekapGWPBU/` + guid;
}
 ...
 * @returns LIST
 */
async function getRekapGwpBadanUsaha() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(
            `${APP_URL}/dashboard/api/report-get-rekap-gwp-badan-usaha?SecretKey=${SECRET_KEY}`,
            {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
async function renderPageNumber() {
    try {
        const req = await getRekapGwpBadanUsaha();
        const result = req.RekapGWPBuNewBusinessRenewalResult;
        let dataSeries = [];
 ...
 * @returns VIEW
 */
async function rendereRekapGwpBadanUsaha() {
    const req = await getRekapGwpBadanUsaha();
    const result = req.RekapGWPBuNewBusinessRenewalResult;
    ...
    const tbody = document.querySelector("#tableRekapGwpBadanUsaha tbody");

    let seqNo = 1;
    result.ListItemRekapGWP.forEach((item) => {
        let rows =
            `<th style="border: 1px solid; text-align: center; background:#EEEEEE; font-weight: normal;">${item.Item29}</th>` +
            `<th style="border: 1px solid; text-align: left; background:#EEEEEE; font-weight: normal;">${item.Name.toUpperCase()}</th>` +
            `<th style="border: 1px solid; text-align: center; background:#EEEEEE; font-weight: normal;">${item.Item27}</th>` +
            `<th style="border: 1px solid; text-align: left; background:#EEEEEE; font-weight: normal;">${item.Item28}</th>` +
    ...
    const tfoot = document.querySelector('#tableRekapGwpBadanUsaha tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item14}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item15}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item16}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item17}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item18}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item19}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item20}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item21}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item22}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item23}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item24}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item25}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; font-weight:bold; z-index:0">${result.TotalRekapGWP.Item26}</td>`;
