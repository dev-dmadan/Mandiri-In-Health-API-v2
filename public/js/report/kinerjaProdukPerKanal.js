...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListKinerjaProductPerKanal();
...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaProductPerKanal").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Produk PerKanal' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaProdukPerKanal/` + guid;
}
 ...
 * @returns LIST
 */
async function getKinerjaProductPerKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-product-per-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListKinerjaProductPerKanal() {
    const req = await getKinerjaProductPerKanal();
    const result = req.KinerjaProdukPerkanalResult;
    ...
    const tbody = document.querySelector('#tableKinerjaProductPerKanal tbody');
    ...
    result.ListItem.forEach(item => {
        let rows =
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item3, item.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item6, item.Item5)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            ...
            `<td style="border: 1px solid; text-align: center"><div style="background-color: ${progressBarColor(item.Item9, item.Item8)};border-radius:50%;width:15px;height:15px;"></div></td>`
    ...
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"  width="1%"><div style="background-color: ${progressBarColor(result.Total.Item3, result.Total.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"  width="1%"><div style="background-color: ${progressBarColor(result.Total.Item6,  result.Total.Item5)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"  width="1%"><div style="background-color: ${progressBarColor(result.Total.Item9,  result.Total.Item8)};border-radius:50%;width:15px;height:15px;"></div></td>`
