...
function printExcel() {
    let tableData = document.getElementById("tableActualPerKanal").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Actual GWP PerKanal' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportActualPerKanal/` + guid;
}
 ...
 * @returns LIST
 */
async function getActualPerKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-actual-per-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
/**
 * RENDER LIST DETAIL AKTUAL PER KANAL
 * @returns VIEW
 */
async function renderListActualPerKanal() {
    const req = await getActualPerKanal();
    const result = req.ActualPerKanalResult;
    ...
    const tbody = document.querySelector('#tableActualPerKanal tbody');
    ...
    const tfoot = document.querySelector('#tableActualPerKanal tfoot');
    let rowsTotal =
