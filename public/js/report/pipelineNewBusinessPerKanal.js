...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListPipelineNewBusinessPerKanal();
...
function printExcel() {
    let tableData = document.getElementById("tablePipelinenNewBusinessPerKanal").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Rekap Pipeline New Business Per Kanal' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/pipelineNBRekapPerBulan/` + guid;
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessPerKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-rekap-pipeline-newbusiness-per-kanal?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessPerKanal() {
    const req = await getPipelineNewBusinessPerKanal();
    const result = req.RekapPipelineNewBusinessPerKanalResult;
    ...
    const tbody = document.querySelector('#tablePipelinenNewBusinessPerKanal tbody');
