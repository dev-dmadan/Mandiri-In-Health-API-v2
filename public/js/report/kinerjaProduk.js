...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaProduct").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Produk' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaProduk/`+ guid;
}
...
/**
 * GET DATA KINERJA PER PRODUK
 * @returns LIST
 */
async function getKinerjaProduct() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-product?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderFilterProduct() {
    try {
        ...
        var dataProduk = result.ListProduct;

        dataProduk = dataProduk.filter(item =>
            item.Name != "HOSPITAL CASH PLAN (INSTIVIDUAL)" &&
            item.Name != "ACCIDENT PROTECTION (INSTIVIDUAL)" &&
            item.Name != "LIFE PROTECTION (INSTIVIDUAL)" &&
            item.Name != "INTRACOINS" 
        );
...
/**
 * RENDER LIST KINERJA PER PRODUK
 * @returns VIEW
 */
async function renderListKinerjaProduct() {
    const req = await getKinerjaProduct();
    ...
    const tbody = document.querySelector('#tableKinerjaProduct tbody');
    ...
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item3, result.Total.Item2)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item6, result.Total.Item5)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; font-weight:bold"><div style="background-color: ${progressBarColor(result.Total.Item9, result.Total.Item8)};border-radius:50%;width:15px;height:15px;"></div></td>`
