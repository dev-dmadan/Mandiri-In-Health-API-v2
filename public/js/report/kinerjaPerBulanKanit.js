...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderFilterMonth();
    renderFilterTahun();
    renderListKinerjaPerbulanKanit();
...
function printExcel() {
    let tableData = document.getElementById("tableKinerjaPerbulanKanit").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Kinerja Perbulan Kepala Unit' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/reportKinerjaPerBulanKanit/` + guid;
}
...
/**
 * GET DATA KINERJA PERBULAN KANIT
 * @returns LIST
 */
async function getKinerjaPerbulanKanit() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-kinerja-perbulan-kanit?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
/**
 * RENDER LIST DETAIL KINERJA PERBULAN KANAL
 * @returns VIEW
 */
async function renderListKinerjaPerbulanKanit() {
    const req = await getKinerjaPerbulanKanit();
    const result = req.KinerjaPerbulanKanitResult;
    ...
    const tbody = document.querySelector('#tableKinerjaPerbulanKanit tbody');
