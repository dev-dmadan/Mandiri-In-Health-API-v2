...
function printExcel() {
    let tableData = document.getElementById("tableLeadingIndicatorKaUnit").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Leading Indicator Ka. Unit' + '.xls'
    a.click();
}
...
function reload() {
    window.location = `${APP_URL}/report/view/leadingIndicatorKAUnit/` + guid;
}
...
async function getLeadingIndicatorKanal() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-leading-indicator-kanit?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
...
async function renderFilterKanit() {
    try {
        ...
        const dataKanit = result.ListKepalaKanit.filter(item => item.KanalSekarangId == KANALDISTRIBUSIID);
        // console.log(result);
...
async function renderListLeadingIndicatorKanal() {
    ...
    const result = req.LeadingIndicatorNewBusinessKanitResult;

    if (!result.Status.Success) {
        throw result.Status.Message;
    }

    const tbody = document.querySelector('#tableLeadingIndicatorKaUnit tbody');
    
    let seqNo = 1;
    result.ListItem.forEach(item => {
        let rows =
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: center">${seqNo}</th>` +
            `<th style="border: 1px solid; background:#EEEEEE; font-weight: normal; text-align: left">${item.Name}</th>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item1}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item2}</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item3}%</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item3)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item4}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item5}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item6}%</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item6)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item7}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item8}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item9}%</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item9)};border-radius:50%;width:15px;height:15px;"></div></td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item10}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: right">${item.Item11}</td>` + 
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center">${item.Item12}%</td>` +
            `<td style="border: 1px solid; background:#EEEEEE; text-align: center"><div style="background-color: ${progressBarColor(item.Item12)};border-radius:50%;width:15px;height:15px;"></div></td>`
        let tr = document.createElement('tr');
        tr.innerHTML = rows;
        tbody.appendChild(tr);
        seqNo++;
    });
    const tfoot = document.querySelector('#tableLeadingIndicatorKaUnit tfoot');
    let rowsTotal =
        ...
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item1}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item2}</td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item3}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold; width: 10px; min-width: 10px; max-width: 10px;"><div style="background-color: ${progressBarColor(result.Total.Item3)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item4}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item5}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item6}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold; width: 10px; min-width: 10px; max-width: 10px;"><div style="background-color: ${progressBarColor(result.Total.Item6)};border-radius:50%;width:15px;height:15px;"></div></td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; z-index:0; font-weight:bold">${result.Total.Item7}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; z-index:0; font-weight:bold">${result.Total.Item8}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item9}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold; width: 10px; min-width: 10px; max-width: 10px;"><div style="background-color: ${progressBarColor(result.Total.Item9)};border-radius:50%;width:15px;height:15px;"></div></td>` +
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; z-index:0; font-weight:bold">${result.Total.Item10}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: right; color:white; z-index:0; font-weight:bold">${result.Total.Item11}</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold">${result.Total.Item12}%</td>`+ 
        `<td style="border: 1px solid black; background-color: #4472C4; text-align: center; color:white; z-index:0; font-weight:bold; width: 10px; min-width: 10px; max-width: 10px;"><div style="background-color: ${progressBarColor(result.Total.Item12)};border-radius:50%;width:15px;height:15px;"></div></td>`
