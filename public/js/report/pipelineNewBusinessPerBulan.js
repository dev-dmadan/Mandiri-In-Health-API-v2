...
document.addEventListener('DOMContentLoaded', function () {
    ...
    renderListPipelineNewBusinessPerBulan();
...
function printExcel() {
    let tableData = document.getElementById("tablePipelinenNewBusinessPerBulan").outerHTML;
    tableData = tableData.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
    tableData = tableData.replace(/<input[^>]*>|<\/input>/gi, ""); //remove input params

    let a = document.createElement('a');
    a.href = `data:application/vnd.ms-excel, ${encodeURIComponent(tableData)}`
    a.download = 'Rekap Pipeline New Business Per Bulan' + '.xls'
    a.click();
}
 ...
 * @returns LIST
 */
async function getPipelineNewBusinessPerBulan() {
    try {
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/report-get-rekap-pipeline-newbusiness-per-bulan?SecretKey=${SECRET_KEY}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                Tahun: TAHUN,
 ...
 * @returns VIEW
 */
async function renderListPipelineNewBusinessPerBulan() {
    const req = await getPipelineNewBusinessPerBulan();
    const result = req.RekapPipelineNewBusinessPerBulanResult;
    ...
    const tbody = document.querySelector('#tablePipelinenNewBusinessPerBulan tbody');
