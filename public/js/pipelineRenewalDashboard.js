...
async function renderListTopBuLapse() {
    ...
    const tfoot = document.querySelector('#tableTopBuLapse tfoot');
    let rowsTotal =
        `<td></td>` +
        `<td class="text-center">TOTAL</td>` +
        `<td class="text-right">${result.Total.Item1}</td>` +
...
async function renderListRekapPipelineRenewal() {
    ...
    result.ListItem.forEach(item => {
        // lapse[7,8] closing[3,4] inprogres[5,6] total[1,2]
        let rows =
            `<td class="text-center">${seqNo++}</td>` +
            `<td>${item.Name}</td>` +
    ...
    const tfoot = document.querySelector('#tableRekapPipelineRenewal tfoot');
    let rowsTotal =
